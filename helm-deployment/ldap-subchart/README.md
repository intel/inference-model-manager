...
Created based on [OpenLDAP helm chart](https://github.com/helm/charts/tree/master/stable/openldap)

It includes the following changes:
- customLdifFiles are stored in a secret instead of a configmap record
- added option to configure custom TLS certificates imported from existing secret set in `certs_secret` parameter or from files set in `cert_files`.
- accounts from ldif config can have autogenerated random passwords. The chart is replacing phrase `*password*` in the ldif files.
...
| `certs_secret`                     | Secret name to store custom TLS certs. It can be predefined or imported from `cert_files` | None     |
| `cert_files`                       | If set, it populates a secret with TLS certificate. File names should match the names set in [LDAP TLS options](https://github.com/osixia/docker-openldap#tls) | None |
